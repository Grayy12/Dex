    ...
    The main explorer interface
		...
		local function main()
			...
			Explorer.InitRightClick = function()
				...
				context:Register(
					"CALL_FUNCTION",
					{ Name = "Call Function", IconMap = Explorer.ClassIcons, Icon = 66, OnClick = function() end }
				)
  ...
  return search]==]
			...
			local function isActuallyVisible(object, isParent)
				...
				elseif object:IsA("ImageLabel") or object:IsA("ImageButton") then
					if (object.ImageTransparency or 0) >= 1 and (object.BackgroundTransparency or 0) >= 1 then
						return false
					end
			...
			local function getHoveredGuiObj()
				local selected = nil
				for _, guiObject in localPlayer.PlayerGui:GetGuiObjectsAtPosition(mouse.X, mouse.Y) do
					if isActuallyVisible(guiObject) then
						selected = guiObject
						break
					end
				end
			...
			game:GetService("UserInputService").InputBegan:Connect(function(input, gameProcessed)
				...
				if input.UserInputType == Enum.UserInputType.MouseButton1 and _clickPartToSelect then
					if _holdingCtrl then
						local selected = getHoveredGuiObj()
						if selected then
							selectObj(selected)
							return
						end
				...
				local function createFrame(self)
					...
					scrollThumbFrame.InputBegan:Connect(function(input)
						...
						local function doTick()
							...
							elseif
								scrollDir == 1
								and mouse[dir] >= scrollThumb.AbsolutePosition[dir] + scrollThumb.AbsoluteSize[dir]
							then
								self:ScrollTo(self.Index + scrollSize)
							end
						end
	...
	The main properties interface
		...
		local function main()
				...
				if #sList > 0 then
					for i = 1, #propList do
						...
						if not prop.IsTag then
							...
							local typeName = typeData and typeData.Name or "string"
			...
			Properties.ShowExplorerProps = function()
				local sList = Explorer.Selection.List
				local foundClasses = {}
				local propCount = 1 -- Use this to index into the 'props' table directly
				...
				local classLists = {} -- Still needed for Properties.ClassLists
				local lower = string.lower
				...
				local attrCount = 0 -- Counter for added attributes

				local showingTags = Settings.Properties.ShowTags -- Assuming this setting exists

				table.clear(props) -- Clear the main props list

				-- Pass 1: Collect regular class properties and build classLists
				for i = 1, #sList do
					local node = sList[i]
					local obj = node.Obj
					local class = node.Class
					...
					while apiClass do
						...
						if not foundClasses[APIClassName] then
							local apiPropsList = indexableProps[APIClassName] -- Use a different name to avoid confusion
							if not apiPropsList then
								apiPropsList = Properties.GetIndexableProps(obj, apiClass)
								indexableProps[APIClassName] = apiPropsList
							end

							if apiPropsList then -- Ensure GetIndexableProps returned a list
								for k = 1, #apiPropsList do
									local propItem = apiPropsList[k] -- Use different name
									local propItemTags = propItem.Tags
									if
										(not propItemTags.Deprecated or showDeprecated)
										and (not propItemTags.Hidden or showHidden)
									then
										props[propCount] = propItem
				...
				Properties.ClassLists = classLists -- Set classLists for other functions

				-- Pass 2: Collect attributes for all selected objects
				if showingAttrs and #sList > 0 then
					local uniqueAttributesData = {} -- Key: AttrName, Value: { firstType = type, prop = attrProp, isMultiType = false }

					for i = 1, #sList do
						local node = sList[i]
						local obj = node.Obj
						local currentObjectAttributes = getAttributes(obj)

						for name, val in pairs(currentObjectAttributes) do
							local typ = typeof(val)
							if not uniqueAttributesData[name] then
								local categoryType = (typ == "Instance" and "Class")
									or (typ == "EnumItem" and "Enum")
									or "Other"
								local valType = { Name = typeNameConvert[typ] or typ, Category = categoryType }
								uniqueAttributesData[name] = {
									firstType = typ,
									prop = {
										IsAttribute = true,
										Name = "ATTR_" .. name,
							...
							elseif
								uniqueAttributesData[name].firstType ~= typ
								and not uniqueAttributesData[name].isMultiType
							then
								uniqueAttributesData[name].isMultiType = true
								uniqueAttributesData[name].prop.MultiType = true
								uniqueAttributesData[name].prop.Tags.ReadOnly = true
								uniqueAttributesData[name].prop.ValueType = { Name = "string" }
							end
						end
					end

					for name, attrData in pairs(uniqueAttributesData) do
						if attrCount < maxAttrs then
							props[propCount] = attrData.prop
							propCount = propCount + 1
				...
				-- Pass 3: Collect tags for all selected objects
				if showingTags and #sList > 0 then
					local uniqueTagNamesAcrossSelection = {}
					for i = 1, #sList do
						local node = sList[i]
						local obj = node.Obj
						local tagsForObj = CollectionService:GetTags(obj)
						for _, tagName in ipairs(tagsForObj) do
							uniqueTagNamesAcrossSelection[tagName] = true
						end
					end
					for tagName, _ in pairs(uniqueTagNamesAcrossSelection) do
						local tagProp = {
							IsTag = true,
							Name = "TAG_" .. tagName,
							TagName = tagName,
				...
				-- Pass 4: Add "Add Attribute" and "Add Tag" buttons if selection exists and settings allow
				if #sList > 0 then
					if showingAttrs then
						props[propCount] = Properties.AddAttributeProp
						propCount = propCount + 1
					end
					if showingTags then
						props[propCount] = Properties.AddTagProp
						propCount = propCount + 1
					end
				end

				-- Pass 5: Sort all collected items
				...
				-- Pass 6: Compute conflicts (only for non-tag, non-attribute props if necessary, current handles it)
				Properties.ComputeConflicts() -- ComputeConflicts already skips tags, ensure it handles attributes correctly if they have sub-props (they don't typically)
			...
			Properties.DisplayAddAttributeWindow = function(editAttr)
				...
				if not win then
					...
					saveButton.OnClick:Connect(function()
						...
						local oldAttributeName = Properties.EditingAttribute
								and Properties.EditingAttribute.AttributeName
							or nil
				...
				if editAttr then
					...
					local typeIndex = table.find(Properties.AllowedAttributeTypes, currentType)
						or table.find(Properties.AllowedAttributeTypes, Properties.TypeNameConvert[currentType])
						or 1
					...
					if prop then
						...
						if prop.SpecialRow then
							...
							if prop.CategoryName then
								...
								expand.Visible = not prop.IsTag
									and (
										typeData.Category == "DataType" and Properties.ExpandableTypes[typeName]
								...
								if inputProp and inputProp.IsTag then
									inputFullName = "TAG_INPUT." .. inputProp.TagName
								end -- Should not happen as tags aren't inputProp
								...
								if gName == inputFullName and not prop.IsTag then -- Tags don't use inputProp
									...
									then
										...
										inputBox.Size =
											UDim2.new(1 - scale, -offset - endOffset - actionButtonOffset, 0, 22)
							...
							if
								...
								or (
									not prop.IsTag
										and Properties.ExpandableTypes[prop.ValueType and prop.ValueType.Name]
									or Properties.ExpandableProps[gName]
								)
							then
						...
						pcall(function()
							...
							if prop.IsAttribute then
								...
								if type(prevDetail) == "string" and prevDetail ~= attributeName then -- Renaming
									...
									if
										setVal == Properties.DefaultPropValue[typeName]
										and currentAttributeVal ~= nil
									then -- If new val is default, try to keep old val if types match
				...
				local guiItems = create({
					...
					},
					{
						...
						"TextBox",
						{
							...
							Parent = { 3 },
							PlaceholderColor3 = Color3.new(0.39215689897537, 0.39215689897537, 0.39215689897537),
							PlaceholderText = "Search properties or tags",
							Position = UDim2.new(0, 4, 0, 0),
							Size = UDim2.new(1, -24, 0, 18),
							Text = "",
							TextColor3 = Color3.new(1, 1, 1),
